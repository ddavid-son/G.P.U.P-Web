...
public class GraphManager implements Serializable {
    ...
    public GraphManager(List<String> targetList, GraphManager originalGraph) {
        ...
        filterOutAllReferencesToNonExcitingTargets();
    ...
    private void updateTypeAndStateOfTarget(int i) {
        if (targetArray.get(i).getDependsOnNames().size() == 0 &&
            ...
            targetArray.get(i).setType(TargetType.INDEPENDENT);

        else if (targetArray.get(i).getDependsOnNames().size() != 0 &&
                targetArray.get(i).getRequiredFor().size() != 0)
            targetArray.get(i).setType(TargetType.MIDDLE);

        else if (targetArray.get(i).getDependsOnNames().size() == 0)
            targetArray.get(i).setType(TargetType.LEAF);

        else
            targetArray.get(i).setType(TargetType.ROOT);

        if (targetArray.get(i).getType().equals(TargetType.INDEPENDENT) ||
                targetArray.get(i).getType().equals(TargetType.LEAF))
            targetArray.get(i).setState(TargetState.WAITING);
        else
            targetArray.get(i).setState(TargetState.FROZEN);
    }
    ...
    public List<String> getAllRelatedOnDFSWorker(String targetName, Set<String> visited, RelationType relationType) {
        ...
        List<String> targetNeighbours = relationType.equals(CommonEnums.RelationType.REQUIRED_FOR) ?
    ...
    public TargetType getTypeOf(String targetName) {
